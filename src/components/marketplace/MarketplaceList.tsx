...
export default function MarketList({ currentUserEmail, isWidget = false }: MarketListProps) {
  ...
  return (
    <MotionList className={`grid gap-4 md:gap-6 items-start ${isWidget
      ...
      {items.map((item) => (
        <MotionItem
          key={item.id}
          className={`relative overflow-hidden rounded-2xl p-5 md:p-6 border transition-all duration-300 ${item.is_sold
        ...
        >
          <div className="flex justify-between items-start gap-3">
            <h2 className={`font-semibold line-clamp-1 ${isWidget ? 'text-sm' : 'text-lg md:text-xl'} ${item.is_sold ? 'text-text-muted' : 'text-text-primary group-hover:text-primary-light transition-colors'}`}>
              {item.product_title}
            </h2>

            {item.is_sold ? (
              <span className="shrink-0 inline-flex items-center gap-1 bg-surface-hover/80 text-text-muted border border-border text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider">
                <CheckCircle className="w-3 h-3" />
                Sold
              </span>
            ) : (
              <span className="shrink-0 bg-primary/10 text-primary border border-primary/20 text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider">
                Active
              </span>
            )}
          </div>

          <p className={`font-bold mt-2 ${isWidget ? 'text-base' : 'text-2xl mt-3'} ${item.is_sold ? 'text-text-disabled' : 'text-primary'}`}>
            ₹{item.price}
          </p>

          <p className={`text-text-muted mt-2 leading-relaxed ${isWidget ? 'text-xs line-clamp-2' : 'text-sm md:text-base line-clamp-3'}`}>
            {item.description}
          </p>

          <div className="mt-4 pt-3 border-t border-border/60 text-xs text-text-secondary flex flex-col justify-end grow">
